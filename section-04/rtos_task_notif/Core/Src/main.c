...
void StartProducer(void *argument)
{
  ...
  for(;;)
  {
		  ...
		  val++;
		  osThreadFlagsSet(ConsumerHandle, 0x0001);
...
void StartConsumer(void *argument)
{
  ...
  for(;;)
  {
	  ...
	  if (flags != osErrorTimeout && flags != osErrorISR && flags != osErrorResource) {
		  sprintf(str_buf, "Processing: %u\r\n", val);
		  HAL_UART_Transmit(&huart2, (uint8_t *)str_buf, strlen(str_buf), HAL_MAX_DELAY);
	  }
    osDelay(10);
  }
